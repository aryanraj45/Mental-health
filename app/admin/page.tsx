...
import { 
  Shield, Users, AlertTriangle, TrendingUp, Flag, Brain, Heart, Activity, Search, Download,
  Eye, MessageSquare, UserCheck, Phone, Mail, CheckCircle, Zap, FileText, Calendar
} from "lucide-react"
...
const mockStudents: Student[] = [
  { id: "1", name: "Aarav Sharma", email: "aarav.sharma@college.edu", rollNo: "CS2021001", course: "Computer Science", year: 3, avatar: "/placeholder-user.jpg", lastActive: new Date(Date.now() - 2 * 36e5), mentalHealthScore: 75, riskLevel: "medium", sessionsCompleted: 12, moodTrend: "improving", emergencyAlerts: 0, lastAssessment: new Date(Date.now() - 24 * 36e5), radarData: [{ subject: 'Academic', value: 80 }, { subject: 'Social', value: 70 }, { subject: 'Emotional', value: 65 }, { subject: 'Coping', value: 75 }, { subject: 'Sleep', value: 85 }], scoreHistory: [{ month: 'Jan', score: 60 }, { month: 'Feb', score: 65 }, { month: 'Mar', score: 72 }, { month: 'Apr', score: 75 }], activityLog: [{ time: new Date(Date.now() - 2 * 36e5), activity: "Completed a session with Dr. Sharma" }, { time: new Date(Date.now() - 48 * 36e5), activity: "Viewed resource: 'Stress Management'" }] },
  { id: "2", name: "Priya Patel", email: "priya.patel@college.edu", rollNo: "EE2020045", course: "Electrical Engineering", year: 4, avatar: "/placeholder-user.jpg", lastActive: new Date(Date.now() - 5 * 6e4), mentalHealthScore: 45, riskLevel: "high", sessionsCompleted: 8, moodTrend: "declining", emergencyAlerts: 1, lastAssessment: new Date(Date.now() - 3 * 36e5), radarData: [{ subject: 'Academic', value: 40 }, { subject: 'Social', value: 55 }, { subject: 'Emotional', value: 30 }, { subject: 'Coping', value: 50 }, { subject: 'Sleep', value: 60 }], scoreHistory: [{ month: 'Jan', score: 55 }, { month: 'Feb', score: 50 }, { month: 'Mar', score: 48 }, { month: 'Apr', score: 45 }], activityLog: [{ time: new Date(Date.now() - 5 * 6e4), activity: "Started AI Chat session" }, { time: new Date(Date.now() - 72 * 36e5), activity: "Crisis keyword detected in chat" }] },
  { id: "3", name: "Rohan Mehra", email: "rohan.mehra@college.edu", rollNo: "CE2022015", course: "Civil Engineering", year: 2, avatar: "/placeholder-user.jpg", lastActive: new Date(Date.now() - 26 * 36e5), mentalHealthScore: 92, riskLevel: "low", sessionsCompleted: 20, moodTrend: "stable", emergencyAlerts: 0, lastAssessment: new Date(Date.now() - 10 * 24 * 36e5), radarData: [{ subject: 'Academic', value: 95 }, { subject: 'Social', value: 88 }, { subject: 'Emotional', value: 90 }, { subject: 'Coping', value: 92 }, { subject: 'Sleep', value: 85 }], scoreHistory: [{ month: 'Jan', score: 80 }, { month: 'Feb', score: 85 }, { month: 'Mar', score: 88 }, { month: 'Apr', score: 92 }], activityLog: [{ time: new Date(Date.now() - 26 * 36e5), activity: "Joined 'Exam Stress' community group" }] },
  { id: "4", name: "Ananya Singh", email: "ananya.singh@college.edu", rollNo: "BT2021007", course: "Biotechnology", year: 3, avatar: "/placeholder-user.jpg", lastActive: new Date(Date.now() - 15 * 6e4), mentalHealthScore: 25, riskLevel: "critical", sessionsCompleted: 3, moodTrend: "declining", emergencyAlerts: 3, lastAssessment: new Date(Date.now() - 1 * 36e5), radarData: [{ subject: 'Academic', value: 30 }, { subject: 'Social', value: 25 }, { subject: 'Emotional', value: 15 }, { subject: 'Coping', value: 20 }, { subject: 'Sleep', value: 40 }], scoreHistory: [{ month: 'Jan', score: 40 }, { month: 'Feb', score: 35 }, { month: 'Mar', score: 30 }, { month: 'Apr', score: 25 }], activityLog: [{ time: new Date(Date.now() - 15 * 6e4), activity: "Flagged community post" }, { time: new Date(Date.now() - 25 * 36e5), activity: "Emergency alert triggered" }] }
];
...
const moderationQueue = [{ id: "1", type: "Community Post", content: "I don't think I can handle this anymore...", author: "Anonymous", timestamp: new Date(Date.now() - 15 * 6e4), flagReason: "Concerning content", priority: "high" }];
...
export default function AdminDashboard() {
  ...
  const filteredStudents = useMemo(() => mockStudents.filter(student => (student.name.toLowerCase().includes(searchQuery.toLowerCase()) || student.rollNo.toLowerCase().includes(searchQuery.toLowerCase())) && (riskFilter === "all" || student.riskLevel === riskFilter)), [searchQuery, riskFilter]);
            ...
            <Card><Table><TableHeader><TableRow><TableHead>Student</TableHead><TableHead>Risk Level</TableHead><TableHead>Health Score</TableHead><TableHead>Last Active</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{filteredStudents.map((student) => (<TableRow key={student.id}><TableCell><div className="flex items-center gap-3"><Avatar><AvatarImage src={student.avatar} /><AvatarFallback>{student.name.split(' ').map(n=>n[0]).join('')}</AvatarFallback></Avatar><div><div className="font-medium">{student.name}</div><div className="text-sm text-muted-foreground">{student.rollNo}</div></div></div></TableCell><TableCell><Badge variant="outline" className={riskBadgeColors[student.riskLevel]}>{student.riskLevel}</Badge></TableCell><TableCell><span className={riskColors[student.riskLevel]}>{student.mentalHealthScore}</span></TableCell><TableCell>{formatTimeAgo(student.lastActive)}</TableCell><TableCell className="text-right"><Dialog onOpenChange={(open) => !open && setSelectedStudent(null)}><DialogTrigger asChild><Button variant="ghost" size="icon" onClick={() => setSelectedStudent(student)}><Eye className="h-4 w-4" /></Button></DialogTrigger>
                {selectedStudent && <DialogContent className="max-w-4xl"><DialogHeader><DialogTitle>{selectedStudent.name}'s Profile</DialogTitle><DialogDescription>{selectedStudent.course} - Year {selectedStudent.year} | {selectedStudent.email}</DialogDescription></DialogHeader>
                  <div className="grid md:grid-cols-2 gap-6 mt-4"><div className="space-y-4">
                    ...
                    <div><Card><CardHeader><CardTitle>Recent Activity</CardTitle></CardHeader><CardContent className="space-y-3 max-h-[450px] overflow-y-auto">{selectedStudent.activityLog.map((log, i) => (<div key={i} className="flex items-start gap-3"><div className="w-2 h-2 rounded-full bg-primary mt-1.5 shrink-0" /><div className="flex-1"><p className="text-sm">{log.activity}</p><p className="text-xs text-muted-foreground">{formatTimeAgo(log.time)}</p></div></div>))}</CardContent></Card></div>
                  </div>
                </DialogContent>}
